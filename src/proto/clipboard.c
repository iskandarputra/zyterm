...
typedef struct {
    ...
    zt_xcb_void_cookie_t (*create_window)(zt_xcb_connection_t *, uint8_t, zt_xcb_window_t,
                                          zt_xcb_window_t, int16_t, int16_t, uint16_t, uint16_t,
                                          uint16_t, uint16_t, zt_xcb_visualid_t, uint32_t,
                                          const void *);
    int (*flush)(zt_xcb_connection_t *);
    int (*get_file_descriptor)(zt_xcb_connection_t *);
    zt_xcb_intern_atom_cookie_t (*intern_atom)(zt_xcb_connection_t *, uint8_t, uint16_t,
                                               const char *);
    zt_xcb_intern_atom_reply_t *(*intern_atom_reply)(zt_xcb_connection_t *,
                                                     zt_xcb_intern_atom_cookie_t,
                                                     zt_xcb_generic_error_t **);
    zt_xcb_void_cookie_t (*set_selection_owner)(zt_xcb_connection_t *, zt_xcb_window_t,
                                                zt_xcb_atom_t, zt_xcb_timestamp_t);
    zt_xcb_void_cookie_t (*change_property)(zt_xcb_connection_t *, uint8_t, zt_xcb_window_t,
                                            zt_xcb_atom_t, zt_xcb_atom_t, uint8_t, uint32_t,
                                            const void *);
    zt_xcb_void_cookie_t (*send_event)(zt_xcb_connection_t *, uint8_t, zt_xcb_window_t,
                                       uint32_t, const char *);
...
static void reply_selection(zt_sel_request_t *req, zt_xcb_atom_t target, const void *data,
    ...
    g.api.change_property(g.conn, ZT_XCB_PROP_MODE_REPLACE, req->requestor, prop, target,
    ...
    ev.property        = prop;
    g.api.send_event(g.conn, 0, req->requestor, ZT_XCB_EVENT_MASK_NONE, (const char *)&ev);
}
...
static void *worker_main(void *arg) {
    ...
    for (int i = 0; i < ATOMS_END; i++)
        cookies[i] =
            g.api.intern_atom(g.conn, 0, (uint16_t)strlen(k_atom_names[i]), k_atom_names[i]);
    for (int i = 0; i < ATOMS_END; i++) {
        zt_xcb_intern_atom_reply_t *r = g.api.intern_atom_reply(g.conn, cookies[i], NULL);
        if (r) {
            g.atoms[i] = r->atom;
            free(r);
        }
    ...
    g.api.create_window(g.conn, ZT_XCB_COPY_FROM_PARENT, g.window, screen->root, -10, -10, 1, 1,
                        0, ZT_XCB_WIN_CLASS_INPUT_ONLY, screen->root_visual, mask, vals);
    g.api.flush(g.conn);
    ...
    for (;;) {
        ...
        pthread_mutex_unlock(&g.mu);
        if (need_claim) {
            g.api.set_selection_owner(g.conn, g.window, g.atoms[A_CLIPBOARD],
                                      ZT_XCB_CURRENT_TIME);
            g.api.flush(g.conn);
        }
        ...
        for (zt_xcb_generic_event_t *ev; (ev = g.api.poll_for_event(g.conn));) {
