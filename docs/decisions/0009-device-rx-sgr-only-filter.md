...
CUF (`foo ESC[6C bar ESC[3C baz …`); rendering it as spaces keeps the columns
aligned in the managed view. It is safe — spaces cannot move the cursor, write the clipboard, or
drive the terminal — and the count is clamped to the parameter cap. Other cursor/erase finals stay
neutralized.
